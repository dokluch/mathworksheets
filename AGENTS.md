...
- `src/App.jsx` — catalog UI + worksheet switcher. `ICONS` and `COMPONENTS` maps keyed by worksheet id. Provides `LocaleContext`; renders `components/LanguageSwitcher.jsx` (globe button + `role="menu"` of real links, `.no-print`) in the sidebar header and in the top-right corner of the catalog/static pages, `components/StaticPage.jsx` (innerHTML from `staticBody()` in `render.js`) for `/about`, `/privacy`, `/terms`, `/developers`, and `components/SiteFooter.jsx` (`.no-print`) on the catalog, static pages and, in compact form, the worksheet sidebar.
...
- No `Accept-Language` redirect: HTML varies on `Accept` only. The chosen language is stored in localStorage (`app.locale`), never in a cookie; a bare `/` restores it, an explicit URL always wins.
