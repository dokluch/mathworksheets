...
| `npm run screenshots` | Regenerate the README screenshots in `screenshots/` (catalog, Multiplication, Add & Subtract at fixed set numbers) with Playwright from the dev server |

Node 24, npm 11. No TypeScript.

## Layout

- `src/worksheets.js` — **the catalog**: id, slug, label, descriptions, grades, topic, skills, settings, color (English), grouped by topic in the order of `TOPICS` in `src/lib/topics.js`. Everything SEO-related derives from it.
- `src/i18n/` — **i18n**: `locales.js` (`LOCALES` = en, fr, es, de, it, ru, zh; `LOCALE_META`; `splitLocale`/`localizePath`), `index.js` (`t(locale, key, params)` with `{param}` interpolation and `Intl.PluralRules` plural objects, `localizeWorksheet`/`localizePage`), `context.js` (`useT()`/`useLocale()` for React), `all.js` (registers every catalog; imported by the Vite config, build scripts, edge middleware and test setup — never by browser code), `load.js` (`loadLocale(code)`: the browser fetches one catalog on demand; only English is bundled), `messages/<locale>.js` (one nested tree per language; `en.js` is the reference key set, the others add `worksheets.<id>` and `pages.<id>` blocks). English is served at the root, every other locale under `/<locale>/…`.
...
1. Add an entry to `src/worksheets.js` at the end of its topic's run (unique `id` and `slug`, a `topic` from `TOPICS` in `src/lib/topics.js`, `longDesc` ≥ 80 chars, at least one skill and one setting; the array is grouped in `TOPICS` order and `src/lib/topics.test.js` fails otherwise), then add a `worksheets.<id>` block (label, shortDesc, longDesc, skills, settings with the same counts) to every `src/i18n/messages/<locale>.js` except `en.js` (`src/i18n/i18n.test.js` fails until you do).
...
- `WORKSHEETS` is grouped by `topic` in the order of `TOPICS` (`src/lib/topics.js`), so the catalog, the sidebar, the sitemap, `llms.txt` and `worksheets.json` share one order and nothing sorts. The home page (React and static) titles one section per topic, `worksheets.json` carries each sheet's `topic` and a `topics` index, and a new topic needs a `topics.<id>` name in every message file.
