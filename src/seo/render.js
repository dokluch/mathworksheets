...
import { PAGES, findPageBySlug } from '../pages.js'
import {
...
/** About / Privacy / Terms (src/pages.js). */
...
export function routes() {
  return [homeRoute(), ...WORKSHEETS.map(worksheetRoute), developersRoute(), ...pageRoutes()]
}
...
export function findRoute(pathname) {
  ...
  const page = /^\/[a-z0-9-]+$/.test(p) ? findPageBySlug(p.slice(1)) : null
  if (page) return pageRoute(page)
...
/** Plain text with optional `[label](url)` links → escaped HTML with anchors. */
...
/** Same text for Markdown: site-relative links become absolute URLs. */
export function inlineMarkdown(text) {
  return text.replace(LINK_RE, (_, label, href) => `[${label}](${href.startsWith('/') ? absoluteUrl(href) : href})`)
}
  ...
  if (route.kind === 'page') {
    ...
    return {
      ...
      '@graph': [
        ...
        authorNode(),
        breadcrumbs([{ name: BRAND, path: '/' }, { name: page.navLabel, path: route.path }]),
      ],
    }
  }
...
function developersBody() {
  ...
  return `<nav aria-label="Breadcrumb"><a href="/">${BRAND}</a> › Developer Resources</nav>
      <header class="catalog-header">
        <div class="catalog-brand">
...
function sectionHtml(section) {
  ...
  for (const p of section.paragraphs || []) parts.push(`<p>${inlineHtml(p)}</p>`)
  if (section.items?.length) {
    parts.push(`<ul>\n        ${section.items.map(i => `<li>${inlineHtml(i)}</li>`).join('\n        ')}\n      </ul>`)
  }
  return parts.join('\n      ')
}
...
function pageBody(route) {
  const page = route.page
  return `<nav aria-label="Breadcrumb"><a href="/">${BRAND}</a> › ${escapeHtml(page.navLabel)}</nav>
      <header class="catalog-header">
        <div class="catalog-brand">
          <h1 class="catalog-title">${escapeHtml(page.title)}</h1>
          <p class="catalog-subtitle">Last updated ${formatDate(page.updated)}</p>
        </div>
      </header>
      <section class="static-intro">
        <p>${inlineHtml(page.description)}</p>
      </section>
...
export function staticBody(route) {
  if (route.kind === 'developers') return developersBody()
...
function sectionMarkdown(section) {
  ...
  for (const p of section.paragraphs || []) parts.push(inlineMarkdown(p), '')
  if (section.items?.length) parts.push(...section.items.map(i => `- ${inlineMarkdown(i)}`), '')
  return parts.join('\n').trimEnd()
}
...
export function renderPageMarkdown(route) {
  const page = route.page
  const more = [
    { label: `${BRAND} home`, md: '/index.md' },
    ...PAGES.filter(p => p.id !== page.id).map(p => ({ label: p.title, md: pageRoute(p).md })),
  ].map(l => `- ${mdLink(l.label, l.md)}`).join('\n')
  return `# ${page.title}

> ${inlineMarkdown(page.description)}

**Last updated:** ${formatDate(page.updated)}
**URL:** ${absoluteUrl(route.path)}

${page.sections.map(sectionMarkdown).join('\n\n')}
...
function notFoundLinks() {
  return [
    ...
    ...PAGES.map(p => ({ label: p.title, path: pageRoute(p).path })),
