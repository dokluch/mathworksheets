...
describe('scripts/prerender.mjs', () => {
  it('writes every derived file next to the built index.html', async () => {
    ...
    expect(written.length).toBe(2 * (WORKSHEETS.length + 2) + 6)
    expect(logs[0]).toContain(`wrote ${written.length} files`)

    const root = await readdir(dist)
    for (const f of ['index.html', 'index.md', 'llms.txt', 'llms-full.txt', 'sitemap.xml', 'robots.txt', 'worksheets.json', '404.html', 'developers.html', 'developers.md']) {
